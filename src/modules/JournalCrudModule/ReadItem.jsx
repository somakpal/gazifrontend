...
export default function ReadItem({ config }) {
    ...
    if (currentResult) {
        const selectedItem = (items) =>
            items.find((item) => {
                console.log('inSIDE loop', item.id, currentResult.id);
                //return item.id === currentResult.id;
            });
        const myitem = selectedItem(items);
        console.log('somak good readitem bhai BARA', currentResult, currentResult.id, myitem);
    }

    //console.log('somak readitem:', currentResult);
    useEffect(() => {
